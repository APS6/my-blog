import Image from "next/image";
import Link from "next/link";
import { urlForImage } from "@/sanity/lib/image";
import getBlogs from "../utils/getBlogs";
import Body from "@/components/Body";
export async function generateMetadata({ params: { slug } }) {
  const blogs = await getBlogs();
  const blog = blogs.find((blog) => blog.slug.current === slug);

  return {
    title: blog.title + " | Anirudha's blog",
    description: blog.shortDescription,
  };
}

export default async function User({ params: { slug } }) {
  const data = await getBlogs()

  //sorting for new blogs and taking the 3 newest
  const sorted = data.sort(
    (a, b) => new Date(b.publishedAt) - new Date(a.publishedAt)
  );
  const blogs = sorted.slice(0, 3);
  
//getting the specific blog data
  const blog = data.find((blog) => blog.slug.current === slug);
  if (!blog) {
    return <div>Blog not found</div>;
  }
  return (
    <div>
      <div className="post pt-6 border-b-textWhite border-b-2 pb-10">
        <div className="border-b-2 border-b-textWhite pb-14 maindiv">
          <h1 className="text-4xl md:text-6xl md:leading-tight">
            {blog.title}
          </h1>
          <p>{blog.description}</p>
          {blog.mainImage && (
            <Image
              src={urlForImage(blog.mainImage)}
              alt={blog.mainImage.alt}
              width={600}
              height={400}
              className="object-contain m-auto"
            />
          )}
        </div>
        <Body content={blog.body} />
      </div>
      <div className="mt-14 pb-8">
        <h2 className="text-4xl mb-6">New Blogs</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 min-[900px]:grid-cols-3 gap-16">
          {blogs.map((blog) => {
            return (
              <Link href={`/${blog.slug.current}`} key={blog._id}>
                <div className="flex flex-col gap-4 p-4 w-64 h-52 bg-[#09090A] rounded card">
                  <h1 className="text-xl">{blog.title}</h1>
                  <p>{blog.shortDescription}</p>
                  <div className="flex justify-between">
                    <span>{blog.publishedAt}</span>
                    <div>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="1.5rem"
                        height="1.5rem"
                        viewBox="0 0 256 256"
                      >
                        <path
                          fill="currentColor"
                          d="M224 48h-64a40 40 0 0 0-32 16a40 40 0 0 0-32-16H32a16 16 0 0 0-16 16v128a16 16 0 0 0 16 16h64a24 24 0 0 1 24 24a8 8 0 0 0 16 0a24 24 0 0 1 24-24h64a16 16 0 0 0 16-16V64a16 16 0 0 0-16-16ZM96 192H32V64h64a24 24 0 0 1 24 24v112a39.81 39.81 0 0 0-24-8Zm128 0h-64a39.81 39.81 0 0 0-24 8V88a24 24 0 0 1 24-24h64ZM160 88h40a8 8 0 0 1 0 16h-40a8 8 0 0 1 0-16Zm48 40a8 8 0 0 1-8 8h-40a8 8 0 0 1 0-16h40a8 8 0 0 1 8 8Zm0 32a8 8 0 0 1-8 8h-40a8 8 0 0 1 0-16h40a8 8 0 0 1 8 8Z"
                        ></path>
                      </svg>
                    </div>
                  </div>
                </div>
              </Link>
            );
          })}
        </div>
      </div>
    </div>
  );
}
